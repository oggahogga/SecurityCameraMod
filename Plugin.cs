...
namespace SecurityCamMod
{
    ...
    public class Plugin : BaseUnityPlugin
    {
        ...
        void Update()
        {
            try
            {
                if (inRoom)
                {
                    ...
                    int layerMask = 1 << i;
                    bool isColliding = Physics.Raycast(ray, out RaycastHit hit, 0.32f, ~layerMask); // 1f is the distance of the raycast, adjust it as needed
                    if (pointer == null)
                    {
                    ...
                    if (d && !isGripPressed)
                    {
                        ...
                        // Cast a ray from the position of the MainAsset in the direction it's moving
                        if (isColliding)
                        {
                            if (cameras[gripPressCount] != null) // If a camera already exists at this index, destroy it
                            {
                            ...
                            float rotationSpeed = 50.0f;  // Change this value to adjust the sensitivity or speed of rotation
                            cameras[gripPressCount] = Instantiate(asset);
                            cameras[gripPressCount].SetActive(true);
                            Vector3 cameraOffset = new Vector3(0f, -3.5f, 0f); // This would spawn the camera 1 unit below the MainAsset
                            ...
                            gripPressCount = (gripPressCount + 1) % 1; // Cycle gripPressCount between 0, 1, and 2
                            UpdateCameras();
        ...
        void UpdateCameras()
        {
            ...
            mainCamera.depth = -1; // Set main camera to a lower depth
            ...
            for (int i = 0; i < cameraCount; i++)
            {
                if (cameras[i] != null)
                {
                    ...
                    camera.depth = 1; // Ensure secondary camera has a higher depth
